...
- 지각 해시: 64비트 pHash + 64비트 dHash (`phash32-dhash9-luma-v1`)
...
응답은 model, modelId, version, preprocessingVersion, dimension, normalized, embedding, perceptualHashVersion, pHash, dHash를 포함합니다.
version은 모델 commit이며 전처리 버전이 다른 벡터끼리는 같은 검색 인덱스에 혼합하지 마세요.

상품 이미지에는 `POST /v1/region-embeddings`를 사용합니다. 같은 요청 본문으로 전체 벡터와
`fixed-overlap-5-v1`의 중앙·좌상·우상·좌하·우하 영역 5개를 반환합니다. 좌표는 0~1 정규화 값이며
여섯 이미지는 한 배치로 추론합니다.
...
기본 테스트는 resize 및 JPEG 압축 뒤 pHash/dHash의 해밍 거리도 검사합니다. 실제 모델 테스트는 벡터 계약과 HTTP 이미지 입력에 더해 합성 티셔츠·머그 삽입 이미지의 영역 점수 개선을 검사합니다.
이번 고정 합성 평가에서 티셔츠는 0.7974→0.8790, 머그는 0.6931→0.8694였습니다. 실제 마켓 데이터의 정확도 지표는 아닙니다.
