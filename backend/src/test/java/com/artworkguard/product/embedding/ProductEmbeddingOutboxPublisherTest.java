...
class ProductEmbeddingOutboxPublisherTest {
 JdbcTemplate jdbc=mock(JdbcTemplate.class);
 KafkaTemplate<String,String> kafka=mock(KafkaTemplate.class);
 UUID eventId=UUID.randomUUID(),jobId=UUID.randomUUID();
 ProductEmbeddingOutboxPublisher publisher=new ProductEmbeddingOutboxPublisher(jdbc,kafka);
 @Test void marksPublishedOnlyAfterKafkaAcknowledges() {
  when(jdbc.query(anyString(),any(RowMapper.class))).thenReturn(List.of(new ProductEmbeddingOutboxPublisher.Pending(eventId,jobId,1,0,"event")));
  when(kafka.send("product.embedding.requested",jobId.toString(),"event")).thenReturn(CompletableFuture.completedFuture(null));
  publisher.publish();
  var order=inOrder(kafka,jdbc);
  order.verify(kafka).send("product.embedding.requested",jobId.toString(),"event");
 ...
 @Test void brokerFailureSchedulesBoundedRetryAndStopsAtTenAttempts() {
  when(jdbc.query(anyString(),any(RowMapper.class))).thenReturn(List.of(new ProductEmbeddingOutboxPublisher.Pending(eventId,jobId,2,9,"event")));
