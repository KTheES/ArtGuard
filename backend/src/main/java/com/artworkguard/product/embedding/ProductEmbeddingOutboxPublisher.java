...
public class ProductEmbeddingOutboxPublisher {
 ...
 public ProductEmbeddingOutboxPublisher(JdbcTemplate jdbc,KafkaTemplate<String,String> kafka){this.jdbc=jdbc;this.kafka=kafka;}
 ...
 public void publish() {
  var events=jdbc.query("""
   SELECT event_id,job_id,generation,attempts,payload FROM product_embedding_outbox
  ...
  for(var event:events){
   try{
    kafka.send("product.embedding.requested",event.jobId().toString(),event.payload()).get(10,TimeUnit.SECONDS);
    jdbc.update("UPDATE product_embedding_outbox SET published_at=now(),attempts=attempts+1 WHERE event_id=?",event.eventId());
   }catch(Exception e){
    ...
    jdbc.update("""
     UPDATE product_embedding_outbox SET attempts=attempts+1,failed=(attempts+1>=10),
    ...
    if(event.attempts()+1>=10)jdbc.update("""
     UPDATE product_embedding_job SET status='FAILED',error_code='EVENT_PUBLISH_FAILED',updated_at=now()
